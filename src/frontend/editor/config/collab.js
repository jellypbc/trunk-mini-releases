  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(tr, collab) {
        ...
        // console.log("%cAPPLYING!", "background:pink", newState, collab)
        if (newState) {
          // console.log("%cescape hatch, returning the newState", "background:pink", newState)
          return newState
        }
        if (tr.docChanged) {
          // console.log("%cdoc changed", "background:lightBlue", unconfirmedFrom(tr))
...
export function receiveTransaction(state, steps, clientIDs, options) {
  ...
  // console.log("%c COLLAB: clientIDs", "background: lightBlue", clientIDs)
  // console.log("%c COLLAB: ours", "background: lightBlue", ours)
  // console.log("%c COLLAB: collabState.unconfirmed", "background: lightBlue", collabState.unconfirmed)

  // we slice off 1 from unconfirmed steps
  ...
  // console.log("%c [collab] steps:", "background:pink;", steps)
...
// :: (state: EditorState) → ?{version: number, steps: [Step], clientID:
  // union<number, string>, origins: [Transaction]}
